...
resource "azurerm_subnet" "lab02agateway" {
  name                 = "GatewaySubnet"
  resource_group_name  = azurerm_resource_group.rg.name
  virtual_network_name = azurerm_virtual_network.lab02a.name
...
resource "azurerm_virtual_network_gateway" "lab02a" {
  name                = "${local.lab02a_name}-vnetgw-${local.random_str}"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
  ...
  ip_configuration {
    name                          = "${local.lab02a_name}-vnetgw-ipconfig-${local.random_str}"
    public_ip_address_id          = azurerm_public_ip.lab02a.id
    private_ip_address_allocation = "Dynamic"
    subnet_id                     = azurerm_subnet.lab02agateway.id
  }
...
resource "azurerm_local_network_gateway" "lab02a" {
  name                = "${local.lab02a_name}-localgw-${local.random_str}"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
...
resource "azurerm_virtual_machine_extension" "lab02aaad" {
